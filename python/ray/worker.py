...
"""A namedtuple that contains the thread-aware info about the current task.
...
class Worker(object):
    ...
    def get_object(self, object_ids):
        ...
        if len(unready_ids) > 0:
            ...
            current_task_id = self.task_context.current_task_id
    ...
    def submit_task(self,
        ...
        with profiling.profile("submit_task", worker=self):
            ...
            assert not self.task_context.current_task_id.is_nil()
            ...
            task = ray.raylet.Task(
                ...
                args_for_local_scheduler,
                num_return_vals,
                self.task_context.current_task_id,
    ...
    def _process_task(self, task, function_execution_info):
        ...
        """
        assert self.task_context.current_task_id.is_nil()
        assert self.task_context.task_index == 0
        assert self.task_context.put_index == 1
        if not (task.actor_id().is_nil() or task.actor_creation_id().is_nil()):
        ...
        with self.lock:
            ...
            with profiling.profile("task", extra_data=extra_data, worker=self):
                ...
                if not (task.actor_id().is_nil()
                        or task.actor_creation_id().is_nil()):
                    # Don't need to reset task_driver_id for actor tasks and
                    # actor creation tasks. Because the following tasks should
                    # all have the same driver id.
                    self.task_driver_id = ray.ObjectID(NIL_ID)
