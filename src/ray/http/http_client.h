 ...
 public:
  ...
  bool Connect(const std::string &host, int port) {
    ...
    try {
      ...
      RAY_LOG(ERROR) << "HttpSyncClient::Connect failed: " << exc.what();
      is_connected_ = false;
      return false;
    }
  }
  ...
  std::pair<boost::system::error_code, std::string> Get(
    ...
    try {
      ...
      RAY_LOG(ERROR) << "HttpSyncClient::Get failed: " << exc.what();
      is_connected_ = false;
      return std::make_pair(exc.code(), std::string());
    }
  }
  ...
  std::pair<boost::system::error_code, std::string> Post(
    ...
    try {
      ...
      RAY_LOG(ERROR) << "HttpSyncClient::Post failed: " << exc.what();
      is_connected_ = false;
      return std::make_pair(exc.code(), std::string());
    }
  }
