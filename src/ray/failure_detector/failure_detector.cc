...
void FailureDetector::RegisterMaster(const ip::detail::endpoint &target,
  ...
  ioc_.post([=] {
    if (master_) {
      RAY_LOG(INFO) << "[RegisterMaster] "
                    << "master[" << master_->endpoint.to_string()
                    << "] already registered"
                    << ", just ignore the registration of " << target.to_string();
      return;
    }

    master_.reset(new MasterRecord(target, current_sys_time_ms()));
    RAY_LOG(INFO) << "[RegisterMaster] " << target.to_string() << " successfully";
    ...
    send_beacon_timer_.async_wait([this](const boost::system::error_code &error) {
      RAY_CHECK(!error) << "[SendBeacon] " << error.message();
      SendBeacon();
    });
  });
}
...
void FailureDetector::OnPingInternal(const rpc::BeaconMsg &beacon, rpc::BeaconAck *ack) {
  ...
  if (itr == workers_.end()) {
    ...
    if (itr->second.process_start_time < beacon.process_start_time()) {
      RAY_LOG(WARNING) << "[" << __FUNCTION__ << "] "
                       << " worker " << node_id << ", " << node_endpoint.to_string()
      ...
      RAY_LOG(WARNING) << "[" << __FUNCTION__ << "] "
                       ...
                       << beacon.process_start_time() << " < "
                       << itr->second.process_start_time << ", skip.";
      return;
    }
...
bool FailureDetector::EndPingInternal(const Status &err, const rpc::BeaconAck &ack) {
  ...
  if (!(master_->endpoint == master_endpoint)) {
    RAY_LOG(ERROR) << "[" << __FUNCTION__ << "] "
                   << "received beacon ack from unknown master, ignore it, "
...
void FailureDetector::SendBeacon() {
  ...
  RAY_LOG(DEBUG) << "[" << __FUNCTION__ << "] "
                 << "send ping message,"
                 << "from[" << endpoint_from_uint64(beacon.from_addr()).to_string()
                 << "],"
                 << "to[" << endpoint_from_uint64(beacon.to_addr()).to_string() << "],"
                 << "time[" << beacon.time() << "]";
  ...
  auto fd_client = GetFDClient();
  auto status = fd_client->Ping(
      beacon, [cb, timer](const Status &status, const rpc::BeaconAck &reply) {
        RAY_LOG(DEBUG) << "[SendBeacon] callback";
        timer->cancel();
        cb(status, reply);
      });

  if (!status.ok()) {
    RAY_LOG(FATAL) << "[SendBeacon] failed! " << status;
  }
