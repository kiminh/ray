...
void GcsObjectManager::OnNodeRemoved(const ClientID &node_id) {
  ...
  for (const auto &object_id : objects_on_node) {
    auto *object_locations = GetObjectLocationSet(object_id);
    if (object_locations) {
      EraseObjectLocationOfSpecifiedNode(object_locations, node_id);
      ...
      [this, done](const std::unordered_map<ObjectID, ObjectTableDataList> &result) {
        ...
        for (auto &item : result) {
          ...
          for (int index = 0; index < object_list.items_size(); ++index) {
            ...
            RAY_CHECK(
                object_locations->emplace(node_id, object_table_date.timestamp()).second);
          }
        }
...
void GcsObjectManager::EraseObjectLocationOfSpecifiedNode(
    GcsObjectManager::LocationSet *object_locations, const ClientID &node_id) const {
