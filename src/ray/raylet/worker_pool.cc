...
WorkerPool::WorkerPool(int num_worker_processes, int num_workers_per_process,
  ...
  HttpRouter::Register("/worker_pool", 
                       "get WorkerPool info",
...
rapidjson::Document WorkerPool::ToJson(rapidjson::Document::AllocatorType* allocator) const {
  ...
  for (const auto &entry : states_by_lang_) {
    ...
    entry_doc.AddMember("num workers", static_cast<uint64_t>(entry.second.registered_workers.size()), alloc);
    entry_doc.AddMember("num drivers", static_cast<uint64_t>(entry.second.registered_drivers.size()), alloc);
    rapidjson::Document workers_doc(rapidjson::kArrayType, &alloc);
    for (auto &w : entry.second.registered_workers) {
      workers_doc.PushBack(w.second->ToJson(&alloc), alloc);
    }
    ...
    for (auto &w : entry.second.registered_drivers) {
      drivers_doc.PushBack(w.second->ToJson(&alloc), alloc);
    }
    entry_doc.AddMember("drivers", drivers_doc, alloc);
    doc.AddMember(rapidjson::StringRef(Language_Name(entry.first).c_str()), entry_doc, alloc);
  }
