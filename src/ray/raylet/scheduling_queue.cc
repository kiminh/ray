...
rapidjson::Document SchedulingQueue::ToJson(
  ...
  for (size_t i = 0; i < static_cast<int>(ray::raylet::TaskState::kNumTaskQueues); i++) {
    TaskState task_state = static_cast<TaskState>(i);
    queue_doc.AddMember(
        rapidjson::StringRef(GetTaskStateString(task_state)),
        static_cast<uint64_t>(GetTaskQueue(task_state)->GetTasks().size()), alloc);
  }
