...
rapidjson::Document TaskDependencyManager::ToJson(
    rapidjson::Document::AllocatorType *allocator) const {
  rapidjson::Document doc(rapidjson::kObjectType, allocator);
  rapidjson::Document::AllocatorType &alloc = doc.GetAllocator();

  doc.AddMember("task dep map size", static_cast<uint64_t>(task_dependencies_.size()),
                alloc);
  doc.AddMember("task req map size", static_cast<uint64_t>(required_tasks_.size()),
                alloc);
  doc.AddMember("req objects map size", static_cast<uint64_t>(required_objects_.size()),
                alloc);
  doc.AddMember("local objects map size", static_cast<uint64_t>(local_objects_.size()),
                alloc);
  doc.AddMember("pending tasks map size", static_cast<uint64_t>(pending_tasks_.size()),
                alloc);
